...
There are several Python scripts that accompany this package. Those relevant
to the module ``atacr`` are called:
...
.. note::
    ...
    The order in which these programs are called is important. In particular,
    the spectral noise analysis and transfer function calculations need to be 
    performed before correcting earthquake seismograms. Here we roughly follow
    the steps highlighted in the 
...
Here we wish to download one month of data for the station M08A for March 2012. 
...
.. code-block:: 
    ...
    $ atacr_daily_spectra.py -O --figQC --figAverage --debug --start=2012-03-04 --end=2012-03-05 M08A.pkl > logfile
...
This script is used to download the earthquake data, for which we wish to 
clean the vertical component using the transfer functions calculated. 
This script is very similar to ``atacr_download_data.py``, with the addition
of Event and Geometry Settings. 
